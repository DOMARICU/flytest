...
function FlyController:setFlyEnabled(enabled)
    ...
    if not character then 
        -- Warte auf Character falls nicht vorhanden
...
function FlyController:applyMovement()
    ...
    if self.flyInstances and self.flyInstances.alignPosition then
        self.flyInstances.alignPosition.Position = v59
    end
end

function FlyController:applyOrientation()
    if self.flyInstances and self.flyInstances.alignOrientation then
